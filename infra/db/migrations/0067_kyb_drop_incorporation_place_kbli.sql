-- 0067_kyb_drop_incorporation_place_kbli.sql
-- CDD KYB: field "Tempat Pendirian" (business_entities.incorporation_place) dan
-- "KBLI" (business_entities.industry_code) dihapus dari form.
--
-- Kolomnya SENGAJA tidak di-DROP: data lama tetap tersimpan/terbaca untuk audit.
-- Yang berubah: trigger submit tidak lagi mewajibkan incorporation_place terisi,
-- sehingga aplikasi Business baru (yang tidak pernah mengisi kolom itu) bisa
-- di-submit. Selain baris tersebut, fungsi identik dengan 0046.
...
BEGIN
  IF NEW.status = 'SUBMITTED' AND OLD.status <> 'SUBMITTED' THEN
    ...
    ELSIF NEW.type = 'BUSINESS' THEN
      ...
      -- tanpa incorporation_place — field dihapus dari form pada 0067).
