...
let ApplicationsService = class ApplicationsService {
    ...
    async createIndividual(dto, userId, branchId) {
        ...
        try {
            ...
            let personId = null;
            const { rows: found } = await client.query(`SELECT id FROM persons WHERE identity_type = $1 AND identity_number = $2 LIMIT 1`, [dto.identity_type, dto.identity_number]);
            if (found[0])
                personId = found[0].id;
    ...
    async getDetail(appId) {
        const { rows: apps } = await this.pool.query(`SELECT a.*, p.full_name, p.identity_type, p.identity_number, p.signature_uri,
       ...
       FROM documents WHERE application_id=$1 ORDER BY created_at DESC`, [appId]);
        let parties = [];
        if (app.business_id) {
            const q = await this.pool.query(`SELECT bp.id, bp.role, bp.is_active, p.id as person_id, p.full_name, p.identity_type, p.identity_number
    ...
    async screenAndComputeRisk(appId) {
        ...
        if (app.type === "INDIVIDUAL") {
            ...
            const { rows: parties } = await this.pool.query(`SELECT bp.id as party_id, bp.role,
       ...
       WHERE bp.business_id=$1 AND bp.is_active = TRUE`, [app.business_id]);
        ...
        let pepHits = 0, dttotHits = 0, pppspmHits = 0;
      ...
      `, [s.name, s.dob || null, s.nationality || null]);
            for (const c of candidates) {
                if (Number(c.score) < SIMILARITY_THRESHOLD)
                    continue;
                await this.pool.query(`INSERT INTO screening_results (application_id, subject_type, subject_ref, list_type, watchlist_id, matched_name, matched_dob, matched_nationality, score)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`, [
                ...
                if (c.list_type === "PEP")
                    pepHits++;
                if (c.list_type === "DTTOT")
                    dttotHits++;
                if (c.list_type === "PPPSPM")
                    pppspmHits++;
            }
        }
        ...
        try {
            ...
            const missing = e?.response?.missing || [];
        ...
        await this.pool.query(`INSERT INTO application_risk (application_id, risk_score, risk_level, factors, created_at)
     VALUES ($1,$2,$3,$4, now())
     ON CONFLICT (application_id) DO UPDATE SET
       risk_score=EXCLUDED.risk_score,
       risk_level=EXCLUDED.risk_level,
       factors=EXCLUDED.factors,
       created_at=now()`, [appId, score, risk_level, JSON.stringify(factors)]);
    ...
    async addParty(appId, dto) {
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id`, [
        ...
        const party = await this.pool.query(`INSERT INTO business_parties (business_id, person_id, role, is_active)
     VALUES ($1,$2,$3,TRUE)
     ON CONFLICT (business_id, person_id, role) DO UPDATE SET is_active=TRUE
     RETURNING id, business_id, person_id, role, is_active, created_at`, [app.business_id, personId, dto.role]);
        return party.rows[0];
    }
